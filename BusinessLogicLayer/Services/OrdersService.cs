...
public class OrdersService : IOrdersService
{
    ...
    public OrdersService(IOrdersRepository ordersRepository, IMapper mapper, IValidator<OrderAddRequest> orderAddRequestValidator, 
                         ...
                         IValidator<OrderItemUpdateRequest> orderItemUpdateRequestValidator, UsersMicroserviceClient usersMicroserviceClient,
    ...
    public async Task<OrderResponse?> AddOrder(OrderAddRequest orderAddRequest)
    {
        ...
        List<Guid> productIDs = orderAddRequest.OrderItems.Select(x => x.ProductID).ToList();
        foreach (Guid productID in productIDs)
        {
            ProductDTO? product = await _productMicroserviceClient.GetProductByProductID(productID) ?? throw new ArgumentException($"Product with ID {productID} does not exist.");
        }
        ...
        {
            ProductDTO? product = await _productMicroserviceClient.GetProductByProductID(orderItem.ProductID);
            if (product != null)
            {
